...
export const backgroundsData: BackgroundDef[] = rawBackgrounds.map((b: any) => {
  ...
  // Normalize ability keys from long form (strength) to short form (str)
  const normalizedOptions = b.ability_bonuses.options.map((opt: string) => opt.substring(0, 3) as AbilityType);
  const normalizedDistribution: Partial<Record<AbilityType, number>> = {};
  for (const key in b.ability_bonuses.default_distribution) {
    const normKey = key.substring(0, 3) as AbilityType;
    normalizedDistribution[normKey] = b.ability_bonuses.default_distribution[key];
  }
  ...
  return {
    ...
    asiOptions: normalizedOptions,
    defaultDistribution: normalizedDistribution,
...
export const spellsData: SpellDef[] = (Array.isArray(rawSpells) ? rawSpells : (rawSpells as any).spells || []).map((s: any, idx: number) => ({
  id: s.id || s.name?.toLowerCase().replace(/\s+/g, '-') || `spell-${idx}`,
  name: s.name_ua || s.name,
  level: s.level,
  school: s.school,
  castingTime: s.casting_time || s.castingTime || s.actionType || '1 action',
  range: s.range,
  components: s.components,
  duration: s.duration,
  description: s.description_ua || s.description,
...
export const skillsList = [
  { id: 'acrobatics', name_ua: 'Акробатика' },
  { id: 'animal_handling', name_ua: 'Поводження з тваринами' },
  { id: 'arcana', name_ua: 'Магія' },
  { id: 'athletics', name_ua: 'Атлетика' },
  { id: 'deception', name_ua: 'Обман' },
  { id: 'history', name_ua: 'Історія' },
  { id: 'insight', name_ua: 'Проникливість' },
  { id: 'intimidation', name_ua: 'Залякування' },
  { id: 'investigation', name_ua: 'Аналіз' },
  { id: 'medicine', name_ua: 'Медицина' },
  { id: 'nature', name_ua: 'Природа' },
  { id: 'perception', name_ua: 'Уважність' },
  { id: 'performance', name_ua: 'Виступ' },
  { id: 'persuasion', name_ua: 'Переконання' },
  { id: 'religion', name_ua: 'Релігія' },
  { id: 'sleight_of_hand', name_ua: 'Спритність рук' },
  { id: 'stealth', name_ua: 'Прихованість' },
  { id: 'survival', name_ua: 'Виживання' }
];
